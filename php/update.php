...
if (isset($_GET['id'])) {
    // Fetch the contact from the contacts table
    ...
    if (!empty($_POST)) {
        ...
        // Handle image upload
        $image = $contact['image']; // Default to existing image path
        ...
        // Update the record
        $stmt = $pdo->prepare('UPDATE contacts SET id = ?, name = ?, email = ?, phone = ?, title = ?, created = ?, image = ? WHERE id = ?');
        $stmt->execute([$id, $name, $email, $phone, $title, $created, $image, $_GET['id']]);
        $msg = 'Updated Successfully!';
        
        // Update the contact variable to reflect the changes
        $contact['name'] = $name;
        $contact['email'] = $email;
        $contact['phone'] = $phone;
        $contact['title'] = $title;
        $contact['created'] = $created;
        $contact['image'] = $image; // Update the image path
    }
} else {
